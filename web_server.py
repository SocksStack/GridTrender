        ...
        <body class="bg-gray-100">
            ...
            <div class="container mx-auto px-4 py-8">
                ...
                <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-8">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <h1 class="text-3xl font-bold text-gray-800">Spot Strategy Monitor</h1>

                        <select id="symbol-selector" class="p-2 border rounded-md bg-white">

                            <option value="">选择现货交易对...</option>
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <span class="text-sm text-gray-500">想查看合约策略？</span>

                        <a href="/derivatives" class="inline-flex items-center px-4 py-2 rounded-md bg-blue-600 text-white hover:bg-blue-700 transition">
            ...
            <script>
                ...
                function updatePageTitle(symbol) {{
                    ...
                    document.title = `Spot Monitor - ${symbol}`;
...
async def handle_symbols(request):
    ...
    try:
        traders = request.app['traders']
        derivative_traders = request.app.get('derivative_traders') or {}
        symbols = list(traders.keys())
        derivative_symbols = list(derivative_traders.keys())
...
async def start_web_server(traders, derivative_traders=None):
    ...
    app['derivative_traders'] = derivative_traders or {}
...
@auth_required
async def handle_derivative_symbols(request):
    derivative_traders = request.app.get('derivative_traders') or {}
    return web.json_response({"symbols": list(derivative_traders.keys())})


@auth_required
async def handle_derivative_status(request):
    derivative_traders = request.app.get('derivative_traders') or {}
    if not derivative_traders:
        return web.json_response({"error": "no derivative traders running"}, status=404)

    symbol = request.query.get('symbol')
    if not symbol or symbol not in derivative_traders:
        symbol = next(iter(derivative_traders.keys()))

    trader = derivative_traders[symbol]
    snapshot = getattr(trader, 'last_snapshot', None)
    position = getattr(trader, 'position_state', None)
    account_metrics = await trader.exchange.fetch_account_metrics()
    funding_info = await trader.exchange.fetch_funding_rate(symbol)

    uptime_seconds = 0
    start_time = getattr(trader, 'start_time', None)
    if start_time:
        uptime_seconds = int(max(time.time() - start_time, 0))

    funding_rate = None
    next_funding_time = None
    if funding_info:
        funding_rate = funding_info.get('fundingRate')
        next_time_ms = funding_info.get('nextFundingTime')
        if next_time_ms:
            try:
                next_funding_time = datetime.fromtimestamp(int(next_time_ms) / 1000).strftime('%Y-%m-%d %H:%M:%S')
    ...
    status = {
        "market": "derivative",
        "symbol": symbol,
        "current_price": getattr(snapshot, 'close', None),
        "trend_direction": getattr(getattr(snapshot, 'direction', None), 'value', None),
        "trend_strength": getattr(snapshot, 'trend_strength', None),
        "adx": getattr(snapshot, 'adx', None),
        "atr": getattr(snapshot, 'atr', None),
        "donchian_high": getattr(snapshot, 'donchian_high', None),
        "donchian_low": getattr(snapshot, 'donchian_low', None),
        "volatility_ratio": getattr(snapshot, 'volatility_ratio', None),
        "position_size": getattr(position, 'size', 0),
        "position_side": getattr(getattr(position, 'side', None), 'value', None),
        ...
        "margin_balance": account_metrics.get('margin_balance'),
        "unrealized_profit": account_metrics.get('unrealized_profit'),
        "funding_rate": funding_rate,
        "next_funding_time": next_funding_time,
        "leverage": getattr(trader.exchange, 'default_leverage', None),
        "margin_mode": getattr(trader.exchange, 'default_margin_mode', None),
...
@auth_required
async def handle_derivative_dashboard(request):
    html = """<!DOCTYPE html>
<html lang='en'>
<head>
    <meta charset='utf-8'>
    <title>Derivative Strategy Monitor</title>
    <link href='https://cdn.jsdelivr.net/npm/tailwindcss@2.2.19/dist/tailwind.min.css' rel='stylesheet'>
</head>
<body class='bg-gray-100'>
    <div class='container mx-auto px-4 py-8'>
        <h1 class='text-2xl font-bold mb-4'>Derivative Strategy Monitor</h1>
        <div class='bg-white shadow rounded p-4 mb-6'>
            <label for='symbol-selector' class='block text-sm font-medium text-gray-700'>Symbol</label>
            <select id='symbol-selector' class='mt-1 block w-64 border border-gray-300 rounded px-3 py-2'></select>
        </div>

        <div class='grid grid-cols-1 md:grid-cols-2 gap-4'>
            <div class='bg-white shadow rounded p-4'>
                <h2 class='text-lg font-semibold mb-3'>Trend & Volatility</h2>
                <p>Current Price: <span id='current-price'>--</span></p>
                <p>Trend Direction: <span id='trend-direction'>--</span></p>
                <p>Trend Strength: <span id='trend-strength'>--</span></p>
                <p>ADX: <span id='adx'>--</span></p>
                <p>ATR: <span id='atr'>--</span></p>
                <p>Donchian High: <span id='donchian-high'>--</span></p>
                <p>Donchian Low: <span id='donchian-low'>--</span></p>
            </div>
            <div class='bg-white shadow rounded p-4'>
                <h2 class='text-lg font-semibold mb-3'>Position</h2>
                <p>Side: <span id='position-side'>--</span></p>
                <p>Size: <span id='position-size'>--</span></p>
                <p>Entry Price: <span id='entry-price'>--</span></p>
                <p>Stop Loss: <span id='stop-loss'>--</span></p>
                <p>Take Profit: <span id='take-profit'>--</span></p>
                <p>Unrealized PnL: <span id='unrealized-pnl'>--</span></p>
                <p>Funding Rate: <span id='funding-rate'>--</span></p>
                <p>Next Funding: <span id='next-funding'>--</span></p>
            </div>
            <div class='bg-white shadow rounded p-4'>
                <h2 class='text-lg font-semibold mb-3'>Account</h2>
                <p>Equity: <span id='equity'>--</span></p>
                <p>Margin Balance: <span id='margin-balance'>--</span></p>
                <p>Unrealized Profit: <span id='unrealized-profit'>--</span></p>
                <p>Leverage: <span id='leverage'>--</span></p>
                <p>Margin Mode: <span id='margin-mode'>--</span></p>
    ...
    <script>
        ...
        async function loadSymbols() {
            const response = await fetch('/api/derivatives/symbols');
            const payload = await response.json();
            const symbols = payload.symbols || [];
            if (!symbols.length) {
                selector.innerHTML = '<option value="">No symbols</option>';
                return;
            }
            selector.innerHTML = symbols.map(symbol => `<option value="${symbol}">${symbol}</option>`).join('');
            currentSymbol = symbols[0];
        ...
        async function updateStatus() {
            if (!currentSymbol) return;
            const response = await fetch(`/api/derivatives/status?symbol=${currentSymbol}`);
            const data = await response.json();
            ...
            document.getElementById('trend-strength').textContent = format(data.trend_strength);
            document.getElementById('adx').textContent = format(data.adx);
            document.getElementById('atr').textContent = format(data.atr);
            document.getElementById('donchian-high').textContent = format(data.donchian_high);
            document.getElementById('donchian-low').textContent = format(data.donchian_low);

            document.getElementById('position-side').textContent = data.position_side || '--';
            document.getElementById('position-size').textContent = format(data.position_size, 4);
            document.getElementById('entry-price').textContent = format(data.entry_price);
            document.getElementById('stop-loss').textContent = format(data.stop_loss);
            document.getElementById('take-profit').textContent = format(data.take_profit);
            document.getElementById('unrealized-pnl').textContent = format(data.unrealized_pnl);
            document.getElementById('funding-rate').textContent = format(data.funding_rate, 6);
            document.getElementById('next-funding').textContent = data.next_funding_time || '--';

            document.getElementById('equity').textContent = format(data.equity);
            document.getElementById('margin-balance').textContent = format(data.margin_balance);
            document.getElementById('unrealized-profit').textContent = format(data.unrealized_profit);
            document.getElementById('leverage').textContent = data.leverage || '--';
            document.getElementById('margin-mode').textContent = data.margin_mode || '--';
